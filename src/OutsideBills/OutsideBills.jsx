...
const OutsideBills = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 dark:bg-gray-900 rounded shadow">
      <button
        onClick={() => navigate(-1)}
        className="flex items-center mb-4 cursor-pointer text-yellow-500 hover:text-yellow-800 dark:text-yellow-400"
      >
        <FaArrowLeft className="mr-2" />
        পিছনে যান
      </button>

      <h1 className="text-2xl text-center text-yellow-700 dark:text-yellow-400 font-bold mb-4">
        অন্যান্য বিল
      </h1>
      ...
      >
        ...
        <button
          type="submit"
          className="btn col-span-full bg-yellow-500 hover:bg-yellow-600 text-white"
        >
          বিল যোগ করুন
        </button>
      </form>
          ...
          <tbody>
            {dataList.map((item) => (
              <tr key={item._id} className="text-center">
                ...
                <td className="border p-2 text-yellow-600 dark:text-yellow-400 font-bold">
                  ৳ {(item.tk ?? 0).toFixed(2)}
                </td>
