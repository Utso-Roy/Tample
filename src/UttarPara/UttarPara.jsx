...
const UttarPara = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:3000/uttarPara')
            .then(res => {
                if (res.data.success) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (name && tk) {
            ...
            try {
                const res = await axios.post('http://localhost:3000/uttarPara', newData);
    ...
    const handleDelete = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const res = await axios.delete(`http://localhost:3000/uttarPara/${id}`);
                    ...
                    console.error(error);
                    Swal.fire('ত্রুটি!', 'সার্ভার ত্রুটির কারণে মুছে ফেলা যায়নি।', 'error');
                }
            }
        });
    };
        ...
        <div className="max-w-4xl h-[calc(200vh-200px)] overflow-y-auto scroll-smooth mx-auto px-4 py-10 text-gray-800 dark:text-gray-200">
            ...
            <div className="bg-white dark:bg-gray-800 shadow-lg border border-gray-200 dark:border-gray-600 rounded-xl p-6">
                ...
                <div className="text-right mt-6 text-lg font-semibold text-[#065f46] dark:text-green-300">
                    মোট অনুদান: <span className="text-xl font-bold">৳ {uttarParaTk.toFixed(2)}</span>
                </div>
            </div>
            ...
            <div className="mt-10">
                ...
                <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-600">
                    <table className="table table-zebra w-full">
                        ...
                        <tbody>
                            {dataList.map((item) => (
                                <tr key={item._id} className="hover:bg-green-50 dark:hover:bg-gray-800">
                                    <td className="px-4 py-2 text-[#065f46] dark:text-green-200">{item.date}</td>
                                    <td className="px-4 py-2 text-[#065f46] dark:text-green-200">{item.name}</td>
