...
const PujarKhoroz = () => {
  ...
  const handleSubmit = (e) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        Swal.fire("সফল", "খরচ যোগ হয়েছে!", "success");
        setFormData({ date: "", name: "", tk: "" });
      })
      .catch(() => Swal.fire("ব্যর্থ", "খরচ যোগ করা যায়নি", "error"));
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6  dark:bg-gray-900 rounded shadow">
      ...
      <button
        onClick={() => navigate(-1)}
        className="flex items-center mb-4 cursor-pointer text-red-600 hover:text-red-800 dark:text-red-400"
      >
      ...
      <div className="overflow-x-auto">
        <table className="table w-full text-sm border">
          <thead className="bg-red-100 dark:bg-gray-800">
            <tr>
              <th className="border text-red-700 dark:text-red-300">তারিখ</th>
              <th className="border text-red-700 dark:text-red-300">নাম</th>
              <th className="border text-red-700 dark:text-red-300">টাকা</th>
              <th className="border text-red-700 dark:text-red-300">অ্যাকশন</th>
            </tr>
          </thead>
          <tbody>
            {dataList.map((item) => (
              <tr key={item._id} className="text-center">
                ...
                <td className="border p-2 text-red-700 dark:text-red-400 font-bold">
                  ৳ {(item.tk ?? 0).toFixed(2)}
                </td>
